...
class Channel:
    ...
    __async: Optional[ThreadPoolExecutor]

    parentItem: Optional[MediaItem]
    currentParser: Optional[ParserData]

    mediaUrlJson: JsonPath
    episodeItemJson: JsonPath
    videoItemJson: JsonPath
    folderItemJson: JsonPath
    pageNavigationJson: JsonPath
    pageNavigationJsonIndex: Optional[int]
    ...
    @property
    def _async(self):
        """ Returns an initialized ThreadPoolExecutor that can be used for async work.

        :return: An initialized ThreadPoolExecutor
        """

        if not self.__async:
            self.__async = ThreadPoolExecutor(max_workers=10)
        return self.__async

    @property
    def sort_key(self) -> str:
        return f"{self.sortOrderPerCountry}-{self.channelName}"
    ...
    def process_folder_list(self, parent_item: Optional[MediaItem] = None) -> Optional[List[MediaItem]]:  # NOSONAR
    ...
    def process_video_item(self, item: MediaItem) -> MediaItem:
        ...
        updated_item = data_parser.Updater(item)
